...
$$ $$
R = \frac{m}{w_r}
$$ ここで、$m$はマッチした単語数、$w_t$は 機械翻訳文の単語数、$w_r$は人間による翻訳文の単語数です。

次に、適合率と再現率の調和平均を計算します。METEORでは再現率により高い重みを与えます。

$$
F_{\text{mean}} = \frac{P \cdot R}{\alpha \cdot P + (1 - \alpha) \cdot R}
$$ 通常、$\alpha = 0.9$ が使用され、再現率が精度の9倍重要とされます。

次に、単語の語順を評価するためにチャンク数（マッチした単語の連続した並び）に基づくペナルティを計算します。

$$
\text{Penalty} = \gamma \cdot \left(\frac{\text{chunks}}{m}\right)^\beta
$$ 通常、$\gamma = 0.5$ 、$\beta = 3$が使用されます。
...
$$
\mathbf{E}_s = \text{Encoder}(s)
$$ $$
\mathbf{E}_h = \text{Encoder}(h)
$$ $$
\mathbf{E}_r = \text{Encoder}(r)
$$ ここで、$\mathbf{E}_s, \mathbf{E}_h, \mathbf{E}_r$はトークンごとのベクトルを並べることで得られる行列です。

次に、先ほど得られた可変長行列を、平均プーリングを用いて固定長行列に変換します。

$$
\mathbf{v}_s = \text{Pooling}(\mathbf{E}_s)
$$ $$
\mathbf{v}_h = \text{Pooling}(\mathbf{E}_h)
$$ $$
\mathbf{v}_r = \text{Pooling}(\mathbf{E}_r)
$$
