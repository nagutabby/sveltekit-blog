#!/usr/bin/env bash
# microsoft/fluentui-emoji から2D(Flat)版の絵文字画像を取得し、article用の画像として配置する。
#
# Usage: fetch-emoji-image.sh <EmojiName>   (例: "White Flag"。実際のリポジトリ側のフォルダ名は
#        "White flag" のように先頭のみ大文字のsentence caseだが、大文字小文字は問わず検索する)
#
# 成功時: web/static/content/articles/images/Microsoft-Fluentui-Emoji-Flat-<Name>.512.png
#         と同名.webp を作成し、標準出力に "images/Microsoft-Fluentui-Emoji-Flat-<Name>.512.png"
...
# 大文字小文字を無視して一致するFlatフォルダを探す。
ALL_FLAT_SVGS=$(gh api "repos/microsoft/fluentui-emoji/git/trees/main?recursive=1" --jq '.tree[].path' 2>/dev/null | grep -E '^assets/[^/]+/Flat/[^/]+\.svg$')

if [ -z "$ALL_FLAT_SVGS" ]; then
  echo "ERROR: fluentui-emojiのファイル一覧を取得できませんでした" >&2
  exit 1
fi

MATCH_PATH=$(printf '%s\n' "$ALL_FLAT_SVGS" | grep -iF "/${INPUT_NAME}/Flat/" | head -1)

if [ -z "$MATCH_PATH" ]; then
  echo "NOT_FOUND: fluentui-emoji に '$INPUT_NAME' の2D(Flat)画像が見つかりませんでした" >&2
  exit 1
fi

# MATCH_PATH の実例: assets/White flag/Flat/white_flag_flat.svg
REAL_NAME=$(printf '%s\n' "$MATCH_PATH" | sed -E 's#^assets/(.+)/Flat/[^/]+\.svg$#\1#')
...
curl -sL --max-time 20 "https://raw.githubusercontent.com/microsoft/fluentui-emoji/main/assets/$ENCODED_NAME/Flat/$SVG_FILE" -o "$TMP_SOURCE"
...
mkdir -p "$OUT_DIR"
BASENAME="Microsoft-Fluentui-Emoji-Flat-${HYPHEN}.512"
