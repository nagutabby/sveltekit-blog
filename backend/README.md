...
`SITE_BASE_URL`(既定`https://blog.nagutabby.uk`。ActivityPub Actor識別子の起点になるため値を変更しない), `WEB_BASE_URL`(`/actor/outbox`が`/atom.xml`を取得するweb側のURL。未設定時は`SITE_BASE_URL`と同じ), `FOLLOWER_TABLE_NAME`(既定`Follower`), `RELAY_CONNECTION_TABLE_NAME`(既定`RelayConnection`), `DYNAMODB_ENDPOINT`(`dynamodb-local`向けのエンドポイント上書き。本番では未設定のままにする), `CORS_ALLOWED_ORIGIN`(ContactServiceへのブラウザからの別オリジン呼び出しを許可するオリジン。api.nagutabby.uk運用時は`https://blog.nagutabby.uk`。未設定時はCORSヘッダーを付与しない=同一オリジンのみ)。AWS認証情報/リージョンはSDKのデフォルト解決(環境変数・IAMロール等、[aws-sdk-go-v2/config](https://pkg.go.dev/github.com/aws/aws-sdk-go-v2/config)に従う)。詳細は`.env.example`を参照。
